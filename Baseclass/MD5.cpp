...
//fopen, and sprintf_s is not safe.
...
std::string CMD5Checksum::Final()
{
	...
	for ( int i=0; i < nMD5Size; i++) 
	{
		...
		{
			sprintf_s(buf, 3, "0%x",lpszMD5[i]);
		}
		else 
		{
			sprintf_s(buf, 3, "%x",lpszMD5[i]);
		}
...
std::string CMD5Checksum::GetMD5(FILE* input)
{
	try
	{
...
std::string CMD5Checksum::GetMD5( const std::wstring& strPathFile, int nOffset, int nSize )
{
	FILE* fInput = NULL;
	_wfopen_s(&fInput, strPathFile.c_str(), L"rb");

	if (!fInput)
		return "";
	
	if(fseek(fInput, nOffset, SEEK_SET))
	{
		fclose(fInput);
		return "";
	}
	...
	try
	{
		...
		//checksum the file in blocks of 1024 bytes
		while (nSize > 0 && (nLength = (int)fread(Buffer,1,nBufferSize,fInput)) > 0 )
		{
			nSize -= nLength;
			if(nSize < nBufferSize)
				nBufferSize = nSize;
...
{
	return GetMD5(strFilePath.c_str());
}

std::string CMD5Checksum::GetMD5(LPCSTR lpstrFilePath)
{
	FILE* fInput = fopen(lpstrFilePath,"rb");
...
std::string CMD5Checksum::GetMD5(const std::wstring& wstrFilePath)
{
	return GetMD5(wstrFilePath.c_str());
}

std::string CMD5Checksum::GetMD5(LPCWSTR lpwstrFilePath)
{
	FILE* fInput = NULL;
	_wfopen_s(&fInput, lpwstrFilePath, L"rb");
