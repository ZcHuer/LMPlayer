...
#define MAX_STREAR_SIZE			512*1024*1024			//512MB
...
public:
	virtual HRESULT Alloc(DWORD dwMaxSize) = 0;
	virtual HRESULT Free() = 0;
	virtual LPBYTE  Attach(LPBYTE lpbyBuffer, DWORD dwValidSize, DWORD dwMaxSize) = 0;
	virtual LPBYTE  Detach() = 0;
	virtual LPBYTE  GetBuffer(DWORD dwOffset) = 0;
	virtual DWORD   GetValidSize() = 0;
	virtual void	SetValidSize(DWORD dwSize) = 0;
	virtual DWORD   GetMaxSize() = 0;
};

#ifndef __NO_DATASTREAM_IMPL__
class CDataStream : public IDataStream
{
public:
	CDataStream(void);
	CDataStream(DWORD dwMaxSize);
	CDataStream(LPBYTE lpbyBuffer, DWORD dwMaxSize, DWORD dwValidSize);
	CDataStream(const CDataStream& ref);
	CDataStream& operator=(const CDataStream& ref);
	virtual ~CDataStream(void);

public:
	virtual HRESULT Alloc(DWORD dwMaxSize);
	virtual HRESULT Realloc(DWORD dwMaxSize);
	virtual HRESULT Free();
	virtual LPBYTE  Attach(LPBYTE lpbyBuffer, DWORD dwValidSize, DWORD dwMaxSize);
	virtual LPBYTE  Detach();
	virtual LPBYTE  GetBuffer(DWORD dwOffset);
	virtual DWORD   GetValidSize();
	virtual void	SetValidSize(DWORD dwSize);
	virtual DWORD   GetMaxSize();
...
#define DEF_DATASTREAMBUFFER_SIZE			1024
class CDataStreamEx : public CDataStream
{
public:
typedef WORD STREAMDATATYPE;
	enum EDataType
	{
		EDT_Unknwn = 0,
		EDT_BYTE = 1,
...
public:
	...
	static bool ParseStringW(CDataStreamEx& buf, CStringW& strW);
	static bool ParseStringA(CDataStreamEx& buf, CStringA& strA);
...
#endif//#ifndef __NO_DATASTREAM_IMPL__