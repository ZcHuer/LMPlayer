...
int BlowFishEncrypt(Blowfish *blowfish, const unsigned char* in, size_t siz_i, unsigned char* out, size_t siz_o, int iMode)
{
...
// 	unsigned char *byte_out;
	...
	if (siz_i % 8 == 0) 
	{
		...
		nLen = (int)(siz_i + 8 - siz_i % 8);
	}
	if ((int)siz_o < nLen ) 
		return -1;
	nRet = EncryptByte(blowfish, in, siz_i, out, iMode);
	return nRet;
}

/*Decrypt from Input Buffer to Output Buffer
Returns false if n is multiple of 8*/
int BlowFishDecrypt(Blowfish *blowfish, const unsigned char* in, size_t siz_i, unsigned char* out, size_t siz_o, int iMode)
{
	int nRet;
	if (siz_o < siz_i) return -1;
	nRet = DecryptByte(blowfish, in, siz_i, out, iMode);
	return nRet;
}
