...
namespace QwBase
{
    ...
    BOOL FindProcess(LPWSTR ProcessName)
    {
        ...
        do
        {
            if (!wcsicmp(ProcessName, pe32.szExeFile))
            {
                bRet = TRUE;
                break;
            }
    ...
    void SetWindowRgn(HWND hWnd)
    {
        ...
        ::SetWindowRgn(hWnd, rgnWindow, TRUE);
    }
    ...
    int GetProcessNum(LPWSTR lpName)
    {
        ...
        do
        {
            if (!wcsicmp(lpName, pe32.szExeFile))
            {
                nCount++;
            }
        } while (Process32Next(hProcessSnap, &pe32));
	...
	BOOL KillProcessFromName(CString strProcessName)
	{
		...
		HANDLE hSnapShot = CreateToolhelp32Snapshot(TH32CS_SNAPPROCESS, 0);//PROCESSENTRY32进程快照的结构体
		PROCESSENTRY32 pe;//实例化后使用Process32First获取第一个快照的进程前必做的初始化操作
		...
		strProcessName.MakeLower();//如果句柄有效  则一直获取下一个句柄循环下去
		while (Process32Next(hSnapShot, &pe))
		{	//pe.szExeFile获取当前进程的可执行文件名称
			CString scTmp = pe.szExeFile;
			//将可执行文件名称所有英文字母修改为小写
			scTmp.MakeLower();	//比较当前进程的可执行文件名称和传递进来的文件名称是否相同
								//相同的话Compare返回0
	...
	*/
	...
	void SetAutoRun(HINSTANCE hInstance, BOOL bAuto)
	{
		...
		PathRemoveFileSpecA(chPath_Run);
		PathAppendA(chPath_Run, "\\LeLaunch.exe /AutoRun-LePlayer");
		...
		if (RegOpenKeyExA(HKEY_CURRENT_USER, "Software\\Microsoft\\Windows\\CurrentVersion\\Run", 0, samDesiredOpen, &hKey) != ERROR_SUCCESS)
		{
			FLOG(_T("SetAutoRun open failed return"));
			return;
		}
		if (RegSetValueExA(hKey, "LePlayer", 0, REG_SZ, (BYTE*)chPath_Run, strlen(chPath_Run)) != ERROR_SUCCESS)
		{
			FLOG(_T("SetAutoRun set failed return"));
			return;
		}
	...
	string strE = QwBase::EncryptStr(strphone);
	string strD = QwBase::DecryptStr(strE);
	...
	*/
	...
	void CreateUninstall()
	{
		...
		PathRemoveFileSpecA(chPath_Run);
		PathRemoveFileSpecA(chPath_Run);
		PathAppendA(chPath_Run, "\\LeLaunch.exe");
		...
		wstring version;
		GlobalConfig::GetVersion(version);
		PathAppendA(chVersion, QwBase::ws2s(version).c_str());
		...
		HKEY lsub;
		if (RegCreateKeyA(hKey, "LePlayer", &lsub) != ERROR_SUCCESS)
		{
			FLOG(_T("创建失败"));
			return;
		}
	...
	void SetupRunPath(HINSTANCE hInstance)
	{
		...
		LSTATUS lRes = RegCreateKeyExA(HKEY_CURRENT_USER, "Software\\Microsoft\\Windows\\CurrentVersion\\App Paths\\LenovoVideo", 0, "", 0, samDesiredOpen, NULL, &hKey, 0);
		if (lRes != ERROR_SUCCESS)
			FLOG(_T("SetupRunPath create failed"));

		lRes = RegCreateKeyExA(HKEY_CURRENT_USER, "Software\\Microsoft\\Windows\\CurrentVersion\\App Paths\\LePlayer", 0, "", 0, samDesiredOpen, NULL, &hKey, 0);
		if (lRes != ERROR_SUCCESS)
			FLOG(_T("SetupRunPath create failed"));
	...
	BOOL CreateDeskShortcut()
	{
		...
		PathRemoveFileSpecW(wsExePath);
		PathAppendW(wsExePath, L"\\LeLaunch.exe");
		...
		wsprintf(wPath, L"%s\\%s.lnk", szDir, L"联想视频");
	...
	BOOL CreatLinkToStartMenu()
	{
		...
		WCHAR wPath[MAX_PATH] = { 0 };
		wsprintf(wPath, L"%s\\%s\\%s.lnk", szStartPath, L"联想视频", L"联想视频");
		
		WCHAR wPathDir[MAX_PATH] = { 0 };
		wsprintf(wPathDir, L"%s\\%s", szStartPath, L"联想视频");
	...
	BOOL bExistTaskBar()
	{
		...
		string tmp = szBuffer;
		tmp += "\\Microsoft\\Internet Explorer\\Quick Launch\\User Pinned\\TaskBar\\联想视频.lnk";
	...
	BOOL Taskbarpin()
	{
		...
		else
		{
			...
			PathAppendW(plnk, L"\\联想视频.lnk");
			PathAppendW(wsExePath, L"\\LePlayer.exe");
			CreateShortCut(plnk, wsExePath); 
	...
	void WriteLePlayerProtocol()
	{
		...
		*/
		LSTATUS lRes = RegCreateKeyExA(HKEY_CLASSES_ROOT, "LePlayer", 0, "", 0, samDesiredOpen, NULL, &hKey, 0);
		if (lRes != ERROR_SUCCESS)
			FLOG(_T("SetupRunPath create failed"));
		if (RegSetValueExA(hKey, "", 0, REG_SZ, (BYTE*)"LePlayer Protocol", strlen(chPath_Run)) != ERROR_SUCCESS)
			FLOG(_T("SetupRunPath set failed"));
		if (RegSetValueExA(hKey, "URL Protocol", 0, REG_SZ, NULL, NULL) != ERROR_SUCCESS)
			FLOG(_T("SetupRunPath set failed"));
		
		hKey = NULL;
		lRes = RegCreateKeyExA(HKEY_CLASSES_ROOT, "LePlayer\\DefaultIcon", 0, "", 0, samDesiredOpen, NULL, &hKey, 0);
		if (lRes != ERROR_SUCCESS)
			FLOG(_T("SetupRunPath create failed"));
		...
		hKey = NULL;
		lRes = RegCreateKeyExA(HKEY_CLASSES_ROOT, "LePlayer\\shell", 0, "", 0, samDesiredOpen, NULL, &hKey, 0);
		if (lRes != ERROR_SUCCESS)
			FLOG(_T("SetupRunPath create failed"));
		...
		hKey = NULL;
		lRes = RegCreateKeyExA(HKEY_CLASSES_ROOT, "LePlayer\\shell\\open", 0, "", 0, samDesiredOpen, NULL, &hKey, 0);
		if (lRes != ERROR_SUCCESS)
			FLOG(_T("SetupRunPath create failed"));
		...
		hKey = NULL;
		lRes = RegCreateKeyExA(HKEY_CLASSES_ROOT, "LePlayer\\shell\\open\\command", 0, "", 0, samDesiredOpen, NULL, &hKey, 0);
		if (lRes != ERROR_SUCCESS)
			FLOG(_T("SetupRunPath create failed"));
