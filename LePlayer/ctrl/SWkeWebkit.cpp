...
namespace SOUI
{
    ...
    _wkeToStringW funWkeToStringW = nullptr;
    ...
    BOOL SWkeLoader::Init( LPCTSTR pszDll )
    {
        ...
        funWkeToStringW = (_wkeToStringW)GetProcAddress(m_hModWke,"wkeToStringW");
        if(!m_funWkeInit 
            || !m_funWkeShutdown
    ...
    int SWkeWebkit::OnCreate( void * )
    {
        ...
        m_wkeClient.onTitleChanged =(&SWkeWebkit::OnTitleChanged);
        m_wkeClient.onURLChanged = &SWkeWebkit::OnUrlChanged;
        m_pWebView->setClientHandler(&m_wkeClient);
    ...
    void SWkeWebkit::OnRButtonDown(UINT nFlags, CPoint point)
    {
        SetMsgHandled(FALSE);
    }

    void SWkeWebkit::OnLButtonDown(UINT nFlags, CPoint point)
    {
        SetMsgHandled(FALSE);
    }
    ...
    void SWkeWebkit::OnTitleChanged(const _wkeClientHandler * clientHandler, const wkeString title)
    {

    }

    void SWkeWebkit::OnUrlChanged(const _wkeClientHandler * clientHandler, const wkeString url)
    {
        if (funWkeToStringW)
        {
            wstring sUrl = funWkeToStringW(url);
        }
    }
	...
	BOOL SWkeWebkit::OnAttrUrl( SStringW strValue, BOOL bLoading )
	{
		m_strUrl=strValue;
        if (!bLoading) {
            //m_strUrl = L"http://pic5.qiyipic.com/image/20180129/d5/08/a_100040138_m_601_m10_220_124.jpg";
            //m_strUrl = L"http://search.ppslist.iqiyi.com/data/test/index.html";
            //m_strUrl = L"http://pc.api.epg.lenovo.com.cn/";
            //m_strUrl = L"http://yph.fanchijr.com/test.php?imgurl=http://pic1.qiyipic.com/image/20180211/4d/e0/a_100037101_m_601_m3_220_124.jpg";
            //m_strUrl = L"http://dist.youxiaoad.com/news/123.html";
            m_pWebView->loadURL(m_strUrl);
        }
		return !bLoading;
	}
