...
namespace SOUI
{
	...
	public:
        ...
        virtual HTREEITEM GetFirstChildItem(HTREEITEM hItem) const
        {
			...
			HTREEITEM hTreeitem = (HTREEITEM)m_tree.GetChildItem((HSTREEITEM)hItem, TRUE);
            return hTreeitem;    
        }
        ...
        {
            SASSERT(hItem != ITvAdapter::ITEM_NULL);
			HTREEITEM hTreeiTem = (HTREEITEM)m_tree.GetChildItem((HSTREEITEM)hItem,FALSE); 
			return hTreeiTem;
        }
        ...
        virtual HTREEITEM GetPrevSiblingItem(HTREEITEM hItem) const 
        {
			...
			HTREEITEM hTreeitem = (HTREEITEM)m_tree.GetPrevSiblingItem((HSTREEITEM)hItem);
			return hTreeitem;
        }
        ...
        {
            SASSERT(hItem != ITvAdapter::ITEM_NULL && hItem != ITvAdapter::ITEM_ROOT);
			HTREEITEM hTreeitem = (HTREEITEM)m_tree.GetNextSiblingItem((HSTREEITEM)hItem);
			return hTreeitem;
        }
		...
		{
			SIZE sz = pItem->GetDesiredSize(prcContainer);
		    return sz;
		}
    ...
    public:
        HSTREEITEM InsertItem(const T & data,HSTREEITEM hParent = STVI_ROOT,HSTREEITEM hInsertAfter = STVI_LAST)
        {
            ItemInfo ii={0};
            ii.data = data;
			HSTREEITEM hStreeitem = m_tree.InsertItem(ii, hParent, hInsertAfter);
            return hStreeitem;
        }
