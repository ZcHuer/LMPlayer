...
namespace SOUI
{
    ...
    BOOL SFlyWnd::CreateChildren( pugi::xml_node xmlNode )
    {
        ...
        if(!bRet) 
			return FALSE;
        SToggle *pFlyWnd = FindChildByName2<SToggle>(NAME_SWITCH);
        if(pFlyWnd)
        {
			pFlyWnd->GetEventSet()->subscribeEvent(EventCmd::EventID,Subscriber(&SFlyWnd::OnSwitchClick,this));
        }
        return TRUE;
    }
