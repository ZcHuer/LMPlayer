...
static const OptionDef *find_option(const OptionDef *po, const char *name)
{
    ...
    int len = p ? p - name : strnlen_s(name, MAX_PATH);

    while (po->name) {
        if (!strncmp(name, po->name, len) && strnlen_s(po->name, MAX_PATH) == len)
            break;
        po++;
    }
    return po;
                      ...
                      const char *codec_name)
{
    ...
    const char *base[3] = { getenv("FFMPEG_DATADIR"),
                            getenv("HOME"),
		getenv("FFMPEG_DATADIR")/*FFMPEG_DATADIR*/, };
    ...
    if (is_path) {
        ...
        {
            for (ls = datadir; ls < datadir + strnlen_s(datadir, MAX_PATH); ls++)
                if (*ls == '\\') *ls = '/';

            if (ls = strrchr(datadir, '/'))
            {
                *ls = 0;
                strncat(datadir, "/ffpresets",  sizeof(datadir) - 1 - strnlen_s(datadir, MAX_PATH));
                base[2] = datadir;
            }
        }
#endif
